...
# or the thin wrapper added in this repo:
...
```text
user turn -> typed conversation state -> BM25 retrieval -> ranked recommendations
```

The first extension point is `shopping_copilot/state/conversation.py`, which holds typed session state while preserving compatibility with the organizer's `starter.Agent` interface and `evaluator.local_evaluator`.
